...
# ---- Load Data ----
datap <- read.csv("E&R_Phenotypic_Data_D.sim - phenotype_data.csv", header = T, sep = ",")
str(datap)
...
datap <- datap %>%
  ...
  dplyr :: select(-dead) #remove unneded rows
...
##plotting population size filtering for less than 200 becasue this is the incomplete data from K 

pop_size_data_filtered <- pop_size_data %>%
  filter(pop_size >= 200)

# 2. Plot pop_size vs. generation, colored by replicate
ggplot(pop_size_data_filtered, aes(x = generation, y = pop_size, color = replicate)) +
  geom_point() +
  geom_line() +
  theme_minimal() +
  labs(
...
for (lev in levels_gen[-1]) {   
  ...
  # Optionally, center the dummy variable
...
summary(lmm_result_O) ###if the t value is bigger than |2| it is likely a signifcant effect
isSingular(lmm_result_O)
dim(pseudo_continuous_female_O)
###visualisation of the mixed model
...
ggsave(filename = "/Users/selmasri/Phenotype_data_analysis_feb2025/poportion_O_repl.png",
       plot = poportion_O_repl,
       width = 20,    # less wide, adjust as needed
       height = 10,   # adjust height as desired
       units = "in")

# ---- Visualization of constant regime ----
# ---- Prepare Data ----
...
head(control_data)
# ---- Figure 1: K7 (Gen 1, 2, 7) ----

##make sure with the proportion is plotted and also the sieve expanded(another plot)


# ---- Figure 2: K1 (Gen 1, 2, 8) ----
...
ggsave(filename = "/Users/selmasri/Phenotype_data_analysis_feb2025/my_plot_K1_O1_C7.png",
       plot = plot_save,
       width = 20,    # less wide, adjust as needed
       height = 10,   # adjust height as desired
       units = "in")
##### calculate mean fly size and plot it for easier visualisation for people i can do this using the pseudo continous data
...
print(avg_fly_size)
##anova on avg sieve size
anova_result <- aov(avg_fly_size ~ factor(generation), data = avg_fly_size)
anova_result2 <- aov(avg_fly_size ~ factor(generation) + factor(replicate), data = avg_fly_size)
summary(anova_result)
summary(anova_result2)


shapiro.test(residuals(anova_result2))  # Should be > 0.05 for normality, which is the case

qqnorm(residuals(anova_result))
qqline(residuals(anova_result), col = "red")


leveneTest(avg_fly_size ~ factor(generation), data = avg_fly_size) #p> 0.0 assumption met of homogenity of variance.
...
ggsave(filename = "/Users/selmasri/Phenotype_data_analysis_feb2025/Avg_rep_gen.png",
...
ggsave(filename = "/Users/selmasri/Phenotype_data_analysis_feb2025/Avg_rep_gen_labeled_rep.png",
...
###GGPLOT FOR GEN 9 ALONE BY SIEVER
ggplot(avg_fly_size_gen9, 
       aes(x = siever, y = avg_fly_size, color = factor(replicate))) +
  geom_jitter(width = 0.2, size = 2) +
  labs(title = "Average Fly Size for Generation 9 by Siever",
       x = "Siever",
       y = "Average Fly Size (µm)",
       color = "Replicate") +
  theme_bw() +
...
# Compute marginal means for generation
emm_generation <- emmeans(anova_result_replicates, ~ generation)
print(emm_generation)

# Pairwise comparisons with Tukey correction
pairs(emm_generation, adjust = "tukey")

# Compute effect sizes between generations

eff_size(emm_generation, sigma = sigma(anova_result_replicates), edf = df.residual(anova_result_replicates))


# Show the estimated means
print(emm_generation)

# Compute the contrast: Difference between Generation 1 and 8
contrast_1_vs_8 <- emmeans::contrast(emm_generation, method = "pairwise", adjust = "none")
print(contrast_1_vs_8)

ggplot(avg_fly_size, aes(x = factor(generation), y = avg_fly_size, color = replicate)) +  
  geom_line(aes(group = replicate)) +  
  geom_point(size = 3) +  
  facet_wrap(~replicate) +  
  labs(title = "Mean Fly Size per Replicate Across Generations", 
       x = "Generation", y = "Mean Fly Size (µm)") +  
  theme_minimal()
# Plot the estimated means with confidence intervals
ggplot(as.data.frame(emm_generation), aes(x = as.numeric(generation), y = emmean)) +
  geom_line(color = "blue", size = 1.2) +  
  geom_point(size = 3, color = "black") +  
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.2, color = "black") +
  labs(title = "Estimated Mean Fly Size Across Generations",
       x = "Generation", y = "Estimated Mean Fly Size (µm)") +
  theme_minimal()

ggplot(as.data.frame(emm_generation), aes(x = factor(generation, levels = unique(avg_fly_size$generation)), y = emmean)) +
  geom_line(color = "blue", size = 1.2, group = 1) +  
  geom_point(size = 3, color = "black") +  
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.2, color = "black") +
  scale_x_discrete(breaks = unique(avg_fly_size$generation)) +  # Ensure all generations appear
  labs(title = "Estimated Mean Fly Size Across Generations",
       x = "Generation", y = "Estimated Mean Fly Size (µm)") +
  theme_minimal()
...
##now for constants and cntrl lets see avg fly size
...
ggsave(filename = "/Users/selmasri/Phenotype_data_analysis_feb2025/mean_size_K.png",
       plot = mean_size_K,
       width = 10,    # less wide, adjust as needed
       height = 8,   # adjust height as desired
       units = "in")
...
ggsave(filename = "/Users/selmasri/Phenotype_data_analysis_feb2025/mean_control.png",
       plot = mean_control,
       width = 20,    # less wide, adjust as needed
       height = 10,   # adjust height as desired
       units = "in")
