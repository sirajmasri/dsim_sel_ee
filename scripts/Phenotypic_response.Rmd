...
informative_bins <- datap %>%
  filter(replicate %in% paste0("O", 1:7)) %>%  # Keep only optimized replicates
  group_by(generation, replicate) %>%
...
#compare sievers in one generation
...
residuals <- residuals(anova_result_2)
library(car)
leveneTest(residuals ~ avg_fly_size_ss$generation)
...
avg_fly_optimized <- ggplot(avg_fly_size, aes(x = factor(generation), y = avg_fly_size)) +
  ...
  labs(title = "Average Fly Size per Generation (Increasing selection Replicates 90->80%)", 
       x = "Generation", 
       y = "Mean Fly Size (µm)") +
  theme_bw() +
  theme(
...
avg_fly_optimzed_labeled <- ggplot(avg_fly_size, aes(x = factor(generation), y = avg_fly_size)) +
  geom_boxplot(fill = "blue", alpha = 0.7, outlier.shape = NA) +
